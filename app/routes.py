...
from app.messaging.events import ORDER_CREATED, ORDER_UPDATED, ORDER_STATUS_CHANGED, ORDER_CANCELLED
from app.models import OrderModel, OrderItemModel, OrderEventModel
from app.schemas import Order, OrderCreate, OrderUpdate, OrderStatusUpdate, OrderSummary, OrderStats
...
async def publish_event_safe(request: Request, event_type: str, data: dict):
    ...
    try:
        ...
        if broker and broker.is_connected:
            ...
            print(f"Warning: Message broker not available, event {event_type} not published")
...
async def create_order_event(db: Session, order_id: str, event_type: str, event_data: dict):
...
def search_orders(
        ...
        date_from: Optional[str] = Query(default=None, description="Date début (YYYY-MM-DD)"),
...
async def create_order(
    ...
    try:
        ...
        await create_order_event(db, order_id, "order_created", {
            "customer_id": order.customer_id,
        ...
        await publish_event_safe(request, ORDER_CREATED, {
            "order_id": order_id,
            "customer_id": order.customer_id,
            "total_amount": str(total_amount),
            "status": "pending",
...
async def update_order(
    ...
    try:
        ...
        await create_order_event(db, order_id, "order_updated", {
        ...
        await publish_event_safe(request, ORDER_UPDATED, {
            "order_id": order_id,
            "customer_id": order.customer_id,
            "changes": changes,
...
async def update_order_status(
    ...
    try:
        ...
        await create_order_event(db, order_id, "status_changed", {
        ...
        await publish_event_safe(request, ORDER_STATUS_CHANGED, {
            "order_id": order_id,
            "customer_id": order.customer_id,
            "old_status": old_status,
            "new_status": new_status,
...
async def cancel_order(
    ...
    try:
        ...
        await create_order_event(db, order_id, "order_cancelled", {
            "old_status": old_status,
            "reason": reason
        })
        ...
        await publish_event_safe(request, ORDER_CANCELLED, {
            "order_id": order_id,
            "customer_id": order.customer_id,
            "reason": reason,
        ...
        raise HTTPException(status_code=500, detail="Erreur lors de l'annulation de la commande")
...
def get_order_statistics(
    ...
    try:
        ...
        status_counts = (db.query(OrderModel.status, func.count(OrderModel.id))
        ...
        recent_orders = (db.query(OrderModel)
